#!/bin/bash
# Build libpng fuzzing harness for crash replay: gcc + ASAN + debug info,
...
# preinstall.sh plus libpng's preinstall.sh (autotools, zlib1g-dev).
#
# Required env (defaults for the standard magma container):
#   TARGET   path to /magma/targets/libpng
#   MAGMA    path to /magma/magma
#   OUT      output dir for binaries (writeable)
#   SHARED   shared dir (canaries.raw goes here)
#
# Output binaries land in $OUT/targets/ (libpng_simplified_read_fuzzer).
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

: "${TARGET:=/magma/targets/libpng}"
: "${MAGMA:=/magma/magma}"
: "${OUT:=/magma_out}"
: "${SHARED:=/magma_shared}"
