#!/bin/bash
# Build expat fuzzing harnesses for crash replay: gcc + ASAN + debug info,
...
# preinstall.sh (assumes gcc, g++, autotools, libtool, make, docbook2x and
# all expat build deps are already present).
#
# Required env (sensible defaults for the standard magma container):
#   TARGET   path to /magma/targets/expat (bind-mounted)
#   MAGMA    path to /magma/magma         (bind-mounted)
#   OUT      output dir for binaries (writeable)
#   SHARED   shared dir (used only for canaries.raw, harmless if unused)
#
# Patched sources must already be in $TARGET/repo (apply_patches.sh on host
# before invoking).
#
# Output binaries land in $OUT/targets/. Run with e.g.:
#   ASAN_OPTIONS=symbolize=1:abort_on_error=1 \
#   ASAN_SYMBOLIZER_PATH=$(which llvm-symbolizer) \
#   $OUT/targets/xml_parsebuffer_fuzzer_UTF-8 <crash_input>
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

: "${TARGET:=/magma/targets/expat}"
: "${MAGMA:=/magma/magma}"
: "${OUT:=/magma_out}"
: "${SHARED:=/magma_shared}"
